...
export default class App extends React.Component {
  ...
  get buttonText() {
    if (this.state.isRecording) {
      return '喋り終わったらもう一度押す';
    } else if (this.state.candidateCharm) {
      return '正しくなければもう一度';
    } else {
      return 'ここを押してマイクに喋り、喋り終わったらもう一度押す'
    }
  }
      ...
      <div>
        <div className="ui">
            ...
            {this.state.candidateCharm ?
              <div>
                ...
                <div className="buttons">
                  <button className="btn btn-primary" onClick={this.onDecideCharm.bind(this)}>
                    <span className="glyphicon glyphicon-arrow-down"/>OK
                  </button>
                </div>
              </div>
              : '？'}
          ...
          <div className="results">
            {this.state.charms.length ?
              <table className="table table-bordered table-hover charm">
                ...
                {this.state.charms.map((charm, index) =>
                  <Charm charm={charm} onDelete={(e) => {
                    this.onDeleteCharm(index);
                    e.preventDefault();
                  }}/>
                )}
                </tbody>
            ...
            <div className="csv">
              <p>CHARM.csv</p>
              <textarea cols="30" rows="5" value={this.csv} onFocus={this.selectAllCsv.bind(this)} ref="csvTextArea"
                        readOnly/>
            </div>
